...
- **New DB columns without resetting.** Schema is created once via `Base.metadata.create_all`; new columns won't appear on existing tables. Either delete the DB or add manual `ALTER TABLE` SQL on startup.
